...
def get_yad2_ads():
    ...
    try:
        ...
        with open("/mnt/data/yad2_response.html", "w", encoding="utf-8") as f:
            f.write(response.text[:50000])  # limit to avoid overflow
        if "window.__PRELOADED_STATE__" in response.text:
            print("__PRELOADED_STATE__ found in response.")
            ...
            ads = [ad for ad in ads if isinstance(ad, dict) and ad.get("id")]
            print(f"Found {len(ads)} ads.")
            return ads
        else:
            print("PRELOADED_STATE not found in Yad2 page response.")
    except Exception as e:
        print("Error fetching or parsing Yad2 data:", e)
    return []
...
Price: {price} ₪
Floor: {floor}
View: <a href='{ad_url}'>Listing</a>
WhatsApp: <a href='{wa_link}'>Send</a>"""
