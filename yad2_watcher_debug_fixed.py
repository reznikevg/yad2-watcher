...
def send_telegram_message(message):
    ...
    try:
        print('📤 שולח לטלגרם:', message)
    response = requests.post(url, json=payload)
    print('✅ תגובת טלגרם:', response.status_code, response.text)
    except Exception as e:
        print("Telegram error:", e)

def print('✨ מודעה חדשה נמצאה:', ad.get('title'))
    notify(ad):
    ...
    while True:
        ...
        for ad in new_ads:
            ...
            if ad_hash not in seen_ads:
                ...
                print('✨ מודעה חדשה נמצאה:', ad.get('title'))
    notify(ad)
